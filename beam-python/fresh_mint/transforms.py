...
class AssignTimestamps(beam.DoFn):
    ...
    def process(self, element: Event):
        ...
        try:
            ...
            ts = datetime.fromisoformat(element.timestamp.replace('Z', '+00:00')).timestamp()
...
class FormatWindowResult(beam.DoFn):
    ...
    def process(self, element, window=beam.DoFn.WindowParam):
        ...
        window_start = datetime.fromtimestamp(float(window.start), tz=None).isoformat() + "Z"
        window_end = datetime.fromtimestamp(float(window.end), tz=None).isoformat() + "Z"
